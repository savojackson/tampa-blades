...
const GOOGLE_PLACES_API_KEY = process.env.GOOGLE_PLACES_API_KEY || 'YOUR_GOOGLE_PLACES_API_KEY'; // Get from Google Cloud Console
const OPENWEATHER_API_KEY = process.env.OPENWEATHER_API_KEY || 'YOUR_OPENWEATHER_API_KEY'; // Get from OpenWeatherMap
...
function createToken(user) {
  return jwt.sign({ id: user.id, username: user.username, role: user.role }, JWT_SECRET, { expiresIn: '2h' });
}
...
async function getGooglePlacesData(lat, lng, spotName) {
  try {
    ...
    const searchUrl = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lng}&radius=1000&keyword=${encodeURIComponent(spotName)}&key=${GOOGLE_PLACES_API_KEY}`;
    ...
    if (searchData.results && searchData.results.length > 0) {
      ...
      return {
        ...
        photos: detailsData.result?.photos?.slice(0, 5).map(photo => 
...
function isGoodSkatingWeather(weatherData) {
  ...
  return temp >= 10 && temp <= 30 && windSpeed < 5.5 && 
...
app.post('/api/register', (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE username = ? OR email = ?', [username, email], (err, row) => {
    ...
    db.run('INSERT INTO users (username, email, password, role, verified) VALUES (?, ?, ?, ?, ?)',
...
app.post('/api/events', requireAuth, upload.single('eventPhoto'), (req, res) => {
  const { 
    title, 
    description, 
    date, 
  ...
  if (!title || !description || !date || !startTime || !endTime || !location || !eventType || !contactEmail) {
  ...
  const params = [
    title, description, date, startTime, endTime, location, eventType,
    skillLevel || null, maxParticipants || null, contactEmail, 
    contactPhone || null, cost || 'Free', equipment || null, 
    eventPhotoPath, req.user.username
  ];
...
app.get('/api/events', (req, res) => {
  db.all('SELECT * FROM events WHERE approved = 1 ORDER BY date ASC', [], (err, rows) => {
...
app.get('/api/events/pending', requireAuth, (req, res) => {
  ...
  db.all('SELECT * FROM events WHERE approved = 0 ORDER BY date ASC', [], (err, rows) => {
    if (err) return res.status(500).json({ error: 'Failed to fetch events' });
    res.json({ events: rows });
  });
});
...
app.post('/api/events/:id/approve', requireAuth, (req, res) => {
  if (req.user.role !== 'admin') return res.status(403).json({ error: 'Admin only' });
  db.run('UPDATE events SET approved = 1 WHERE id = ?', [req.params.id], function (err) {
    if (err) return res.status(500).json({ error: 'Failed to approve event' });
...
app.put('/api/messages/:messageId/read', requireAuth, (req, res) => {
  ...
  db.run('UPDATE messages SET read = 1 WHERE id = ? AND receiverId = ?', 
    [messageId, req.user.id], 
    function (err) {
      if (err) return res.status(500).json({ error: 'Failed to mark message as read' });
      res.json({ success: true });
    }
  );
});
...
app.get('/api/messages/unread/count', requireAuth, (req, res) => {
  db.get('SELECT COUNT(*) as count FROM messages WHERE receiverId = ? AND read = 0', 
    [req.user.id], 
    (err, result) => {
      if (err) return res.status(500).json({ error: 'Failed to get unread count' });
...
app.get('/api/skate-spots', (req, res) => {
  db.all('SELECT * FROM skate_spots WHERE approved = 1 ORDER BY rating DESC', [], (err, spots) => {
    if (err) return res.status(500).json({ error: 'Failed to fetch skate spots' });
    res.json({ spots });
  });
});
...
app.get('/api/skate-spots/area', async (req, res) => {
  ...
  try {
    ...
    db.all(dbQuery, [south, north, west, east], async (err, dbSpots) => {
      if (err) return res.status(500).json({ error: 'Failed to fetch database spots' });
...
app.get('/api/skate-spots/:type', (req, res) => {
  ...
  db.all('SELECT * FROM skate_spots WHERE type = ? AND approved = 1 ORDER BY rating DESC', 
    [type], 
    (err, spots) => {
      if (err) return res.status(500).json({ error: 'Failed to fetch skate spots' });
      res.json({ spots });
...
app.post('/api/skate-spots', requireAuth, (req, res) => {
  const { name, type, difficulty, latitude, longitude, description, features, hours } = req.body;
  
  if (!name || !type || !difficulty || !latitude || !longitude) {
    return res.status(400).json({ error: 'Name, type, difficulty, and coordinates are required' });
  }

  const featuresStr = features ? JSON.stringify(features) : null;
  
  db.run(`INSERT INTO skate_spots (name, type, difficulty, latitude, longitude, description, features, hours, submitted_by) 
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [name, type, difficulty, latitude, longitude, description, featuresStr, hours, req.user.id],
    function (err) {
      if (err) return res.status(500).json({ error: 'Failed to submit skate spot' });
      res.json({ 
        success: true, 
        id: this.lastID,
...
app.post('/api/skate-spots/:id/rate', requireAuth, (req, res) => {
  ...
  db.get('SELECT rating, reviews FROM skate_spots WHERE id = ?', [id], (err, spot) => {
    if (err) return res.status(500).json({ error: 'Failed to get skate spot' });
    ...
    db.run('UPDATE skate_spots SET rating = ?, reviews = ? WHERE id = ?',
...
app.get('/api/skate-spots/pending', requireAuth, (req, res) => {
  ...
  db.all('SELECT * FROM skate_spots WHERE approved = 0 ORDER BY created_at DESC', [], (err, spots) => {
    if (err) return res.status(500).json({ error: 'Failed to fetch pending spots' });
    res.json({ spots });
  });
});
...
app.post('/api/skate-spots/:id/approve', requireAuth, (req, res) => {
  ...
  db.run('UPDATE skate_spots SET approved = 1 WHERE id = ?', [req.params.id], function (err) {
    if (err) return res.status(500).json({ error: 'Failed to approve skate spot' });
    res.json({ success: true });
  });
});
...
app.delete('/api/skate-spots/:id', requireAuth, (req, res) => {
  ...
  db.run('DELETE FROM skate_spots WHERE id = ?', [req.params.id], function (err) {
    if (err) return res.status(500).json({ error: 'Failed to delete skate spot' });
    res.json({ success: true });
  });
});
...
app.get('/api/skate-spots/:id/enhanced', async (req, res) => {
  ...
  try {
    db.get('SELECT * FROM skate_spots WHERE id = ?', [id], async (err, spot) => {
      ...
      const googleData = await getGooglePlacesData(spot.latitude, spot.longitude, spot.name);
      
      // Get reviews
      db.all('SELECT r.*, u.username FROM spot_reviews r JOIN users u ON r.user_id = u.id WHERE r.spot_id = ? ORDER BY r.created_at DESC', [id], (err, reviews) => {
        if (err) return res.status(500).json({ error: 'Failed to fetch reviews' });
        
        // Get photos
        db.all('SELECT * FROM spot_photos WHERE spot_id = ? ORDER BY created_at DESC', [id], (err, photos) => {
...
app.post('/api/skate-spots/:id/reviews', requireAuth, (req, res) => {
  ...
  db.run('INSERT INTO spot_reviews (spot_id, user_id, rating, comment, photos) VALUES (?, ?, ?, ?, ?)',
    ...
    function (err) {
      ...
      db.get('SELECT AVG(rating) as avgRating, COUNT(*) as reviewCount FROM spot_reviews WHERE spot_id = ?', [id], (err, result) => {
        if (!err && result) {
          db.run('UPDATE skate_spots SET rating = ?, reviews = ? WHERE id = ?', 
...
app.post('/api/skate-spots/:id/photos', requireAuth, (req, res) => {
  ...
  db.run('INSERT INTO spot_photos (spot_id, user_id, photo_url, caption, source) VALUES (?, ?, ?, ?, ?)',
...
app.get('/api/search-location', async (req, res) => {
  ...
  try {
    ...
    const autocompleteUrl = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(query)}&types=geocode&key=${GOOGLE_PLACES_API_KEY}`;
    ...
    if (autocompleteData.status === 'OK' && autocompleteData.predictions.length > 0) {
      ...
      if (detailsData.status === 'OK' && detailsData.result) {
        ...
        res.status(404).json({ 
          ...
          message: detailsData.error_message || 'Could not get location details'
        });
      }
    } else {
      res.status(404).json({ 
        ...
        message: autocompleteData.error_message || 'No predictions found for this search'
      });
    }
  } catch (error) {
...
app.get('/api/autocomplete', async (req, res) => {
  ...
  try {
    const autocompleteUrl = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(input)}&types=geocode&key=${GOOGLE_PLACES_API_KEY}`;
...
app.post('/api/gallery/photos', requireAuth, (req, res) => {
  ...
  db.run('INSERT INTO gallery_photos (user_id, photo_url, caption, category, tags, location, is_public) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [req.user.id, photoUrl, caption, category || 'general', tagsStr, location, isPublic ? 1 : 0],
    function (err) {
...
app.get('/api/gallery/photos', (req, res) => {
  ...
  db.all(query, params, (err, photos) => {
    ...
    let countQuery = 'SELECT COUNT(*) as total FROM gallery_photos WHERE is_public = 1';
    const countParams = [];
    ...
    db.get(countQuery, countParams, (err, result) => {
      if (err) return res.status(500).json({ error: 'Failed to get photo count' });
...
app.post('/api/gallery/photos/:id/like', requireAuth, (req, res) => {
  ...
  db.get('SELECT * FROM photo_likes WHERE photo_id = ? AND user_id = ?', [id, req.user.id], (err, existing) => {
    if (err) return res.status(500).json({ error: 'Failed to check like status' });
    
    if (existing) {
      // Unlike
      db.run('DELETE FROM photo_likes WHERE photo_id = ? AND user_id = ?', [id, req.user.id], function (err) {
        if (err) return res.status(500).json({ error: 'Failed to unlike photo' });
        
        // Update photo like count
        db.run('UPDATE gallery_photos SET likes = likes - 1 WHERE id = ?', [id]);
        
        res.json({ success: true, liked: false });
      });
    } else {
      // Like
      db.run('INSERT INTO photo_likes (photo_id, user_id) VALUES (?, ?)', [id, req.user.id], function (err) {
        if (err) return res.status(500).json({ error: 'Failed to like photo' });
...
app.post('/api/gallery/photos/:id/comments', requireAuth, (req, res) => {
  ...
  db.run('INSERT INTO photo_comments (photo_id, user_id, comment) VALUES (?, ?, ?)',
...
app.delete('/api/gallery/photos/:id', requireAuth, (req, res) => {
  ...
  db.get('SELECT user_id FROM gallery_photos WHERE id = ?', [id], (err, photo) => {
    if (err) return res.status(500).json({ error: 'Failed to check photo ownership' });
    if (!photo) return res.status(404).json({ error: 'Photo not found' });
    if (photo.user_id !== req.user.id) return res.status(403).json({ error: 'Not authorized to delete this photo' });
...
app.get('/api/admin/users', requireAuth, requireSuperAdmin, (req, res) => {
  ...
  let query = 'SELECT id, username, email, role, verified, created_at FROM users WHERE 1=1';
  const params = [];
  ...
  db.all(query, params, (err, users) => {
    ...
    db.get(countQuery, countParams, (err, result) => {
      if (err) return res.status(500).json({ error: 'Failed to get user count' });
...
app.post('/api/admin/users', requireAuth, requireSuperAdmin, (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE username = ? OR email = ?', [username, email], (err, existing) => {
    if (err) return res.status(500).json({ error: 'Failed to check existing user' });
...
app.put('/api/admin/users/:id/role', requireAuth, requireSuperAdmin, (req, res) => {
  const { id } = req.params;
  const { role } = req.body;
...
app.delete('/api/admin/users/:id', requireAuth, requireSuperAdmin, (req, res) => {
  const { id } = req.params;
  ...
  db.run('DELETE FROM messages WHERE senderId = ? OR receiverId = ?', [id, id]);
  ...
  db.run('DELETE FROM events WHERE submittedBy = (SELECT username FROM users WHERE id = ?)', [id]);
    ...
    db.get('SELECT COUNT(*) as count FROM users WHERE role = "admin"', [], (err, result) => {
      ...
      db.get('SELECT COUNT(*) as count FROM users WHERE role = "super_admin"', [], (err, result) => {
        ...
        db.get('SELECT COUNT(*) as count FROM events', [], (err, result) => {
          ...
          db.get('SELECT COUNT(*) as count FROM skate_spots', [], (err, result) => {
            if (err) return res.status(500).json({ error: 'Failed to get skate spot stats' });
            stats.totalSkateSpots = result.count;
            
            db.get('SELECT COUNT(*) as count FROM gallery_photos', [], (err, result) => {
              if (err) return res.status(500).json({ error: 'Failed to get photo stats' });
              stats.totalPhotos = result.count;
